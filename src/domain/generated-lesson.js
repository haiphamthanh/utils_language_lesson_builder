export const MAX_GENERATED_LESSON_WORDS = 150;
...
function requireText(value, field) {
  if (typeof value !== "string" || value.trim() === "") {
    throw new Error(
      `Generated lesson field "${field}" must be non-empty text.`,
    );
  }
}
...
export function countWords(value) {
  ...
  return [...segmenter.segment(value)].filter((segment) => segment.isWordLike)
    .length;
}
...
function requireFiveExamples(item, field) {
  if (!Array.isArray(item.examples) || item.examples.length !== 5) {
    throw new Error(
      `Generated lesson field "${field}.examples" must contain exactly 5 items.`,
    );
  }
...
export function validateGeneratedLesson(lesson) {
  ...
  for (const [groupName, minimum] of groups) {
    ...
    if (!Array.isArray(items) || items.length < minimum) {
      throw new Error(
        `Generated lesson field "review.${groupName}" requires at least ${minimum} item(s).`,
      );
    }
