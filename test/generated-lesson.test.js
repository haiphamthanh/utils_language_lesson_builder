...
test("generated lesson content is limited to 150 words", () => {
  const lesson = generatedLessonFixture({
    content: Array.from(
  ...
  assert.equal(countWords(lesson.content), 151);
  assert.throws(
    () => validateGeneratedLesson(lesson),
    /content.*at most 150 words.*received 151/,
  );
});

test("generated lesson accepts content at the 150-word boundary", () => {
  const lesson = generatedLessonFixture({
    content: Array.from(
